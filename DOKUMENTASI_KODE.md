...
├── outbond.html          # Halaman outbound packages
...
- Grid layout 5 event cards
- Modal carousel untuk galeri gambar

#### Event Cards (Line 79-156):
- 5 cards dalam 2 baris (2+3)
- Setiap card berisi:
  - Gambar event
  - Label dengan nama event
- Cards dapat diklik untuk membuka galeri

#### Modal Carousel Gallery (Line 159-184):
- Bootstrap modal dengan carousel
- **Fitur Carousel Titles** (Baru Ditambahkan):
  - Judul deskriptif untuk setiap gambar
  - Posisi: di bawah gambar (seperti event card labels)
  - Styling: centered, font-size 1.5rem, font-weight 800
  - Tema judul sesuai dengan gambar event

#### Event Gallery Data (JavaScript):
```javascript
const eventGalleryMap = {
  'Splash Carnival': ['foto/splash-carnival-1.jpg', ...],
  'Hydro DJ Night': ['foto/hydro-dj-night-1.jpg', ...],
  // ... 5 event types total
};
...
const eventTitlesMap = {
  ...
  // ... matching titles for each event
};
```

#### Fungsi Carousel:
- `openImageGallery(imgElement)` - Membuka modal dengan carousel
- Menampilkan multiple gambar per event
- Setiap slide memiliki judul deskriptif
- Navigasi carousel (prev/next buttons)
